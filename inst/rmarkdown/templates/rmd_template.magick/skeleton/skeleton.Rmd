...
The example requires microscopy images to work.

Set the location of your image set (if you have one already):

```{r}
data.dir <- "/path/to/your/data"
```

Otherwise, until you get you own, you may get sample images from the `rcell2.examples` package.
...
> This step is optional, but also part of our usual workflow. The `pdata` CSV file
> holds information about how the cells of each image were prepared and treated.
...
result <- shinyCell(cdata, paths = images, initial_facet = "~pos+t.frame", debug_msgs = T)
```

```{r}
pgn <- read.csv("/tmp/brpts.csv")
...
```{r}
polygon_df_list <- list(pgn)
truthMode = "all"
cell_unique_id_field = "ucid_time"
...
debugonce(polyFilterCell)

polyFilterCell(cdataDF = cdata, 
               filterDF = cfilter, 
               polygonDF = polygon_df_list[[1]],
               polygonName = paste0("polygon", 1)
               ) %>% View()
```
