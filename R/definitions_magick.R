# cellMagick DEFINITIONS ####

#' Add a border to one side of the image
#'
#' image_border_one is good for padding images to add captions later, while magick's original image_border adds borders symmetrically to opposing sides.
#'
...
#' @inheritParams magickCell
#' @inheritDotParams  magickCell
#' @param time_colum Sort cdata by \code{time_colum} first, set to NULL to skip.
#' @param id_column This column is checked for uniqueness of each value.
...
cellStrip <- function(cdata,
  ...
  # Optional sort by t.frame
  if(!is.null(time_colum)) cdata <- dplyr::arrange(cdata, !!as.symbol(time_colum))
  ...
  imgs <- magick::image_append(img, stack = !stack_time_horizontally)
  return(imgs)
}
...
cellSpread <- function(cdata, paths,
  ...
  if(is.factor(x_data) | is.ordered(x_data)){
    # x_data <- methods::as(object = levels(x_data), Class = typeof(x_data))
    x_data <- as.integer(x_data)
  }
  if(is.factor(y_data) | is.ordered(y_data)){
    # y_data <- methods::as(object = levels(y_data), Class = typeof(y_data))
...
#' @param discrete_exp Expansion argument passed to \code{scale_x/y_discrete}, increase to 0.5*1E-2 or more if you see weird artifacts.
...
cellSpreadPlot <- function(cdata, paths,
  ...
  x_data <- cdata[,xvar, drop =T]
  y_data <- cdata[,yvar, drop =T]
  ...
  for(channel in ch){
    ...
    p <- ggplot(cdata.spread,
      ...
      geom_blank() +
      # Set plot limits for continuous x variables only
      {
        if( !(is.factor(x_data) | is.ordered(x_data)) )
          scale_x_continuous(limits = range(cell_tile$x.breaks), expand = expansion())
        else
          scale_x_discrete(expand = expansion(add = discrete_exp))
      } +
      # Set plot limits for continuous y variables only
      {
        if( !(is.factor(y_data) | is.ordered(y_data)) )
          scale_y_continuous(limits = range(cell_tile$y.breaks), expand = expansion())
        else
          scale_y_discrete(expand = expansion(add = discrete_exp))
      } +
      # Customize theme
      theme_minimal() +
      theme(legend.position = "none",
            aspect.ratio=y.cuts/x.cuts)  #, plot.margin = unit(rep(0,4), "mm"))
...
#' @param customize_images Use a custom magick-like function to "customize" the image when TRUE (FALSE by default). Can be a logical vector, each value applied separately to each channel (recycled to the length of \code{ch}).
