...
#' @import shiny formattable dplyr tidyr magick keys
...
tagCellServer <- function(input, output, session) {
  ...
  output$pics2 <- shiny::renderImage({
    ...
    if(nrow(d) > 0 & max.frames > 0) {
      ...
      tmpimage <- cell.strip %>% 
        magick::image_resize(geometry = geometry_size_pixels(width = plot_width * length(frame.range) / max.frames))
  ...
  # Reactive IMAGE 1: cell magick / current cell  ----------------
  ...
  output$plot <- shiny::renderPlot({
    ...
    if(nrow(table_output) > 0){
      ...
      id_data <- d[, c("ucid", "pos", "cellID", reactive_values$click_vars$yvar)] %>% unique()
      table_output <- separate(table_output, ucid_t.frame, c("ucid", "t.frame")) %>% 
        mutate(ucid = as.integer(ucid), 
               t.frame = as.integer(t.frame)) %>% 
        left_join(id_data, by = "ucid") %>% 
